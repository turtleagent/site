...
Required for the current BYO-E2B `/teleport` flow.

```bash
E2B_API_KEY=e2b_...
```

If this is unset, local SuperTurtle usage still works, but `/teleport` cannot launch the remote E2B runtime.
...
## Teleport and Remote Runtime (Advanced)

These variables exist for E2B/webhook runtimes and are usually managed by SuperTurtle itself rather than set manually in normal local use:
...
- `TELEGRAM_WEBHOOK_HEALTH_PATH`
- `TELEGRAM_WEBHOOK_READY_PATH`
- `SUPERTURTLE_RUNTIME_ROLE`
- `SUPERTURTLE_REMOTE_MODE`
- `SUPERTURTLE_RUNTIME_INSTALL_SPEC`
