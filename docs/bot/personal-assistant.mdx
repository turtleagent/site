...
If you run the bot directly with `bun run start` during development, export the same path in your shell before starting so the launcher can find `.superturtle/.env`:
